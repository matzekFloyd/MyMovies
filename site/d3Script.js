...
(function () {
  ...
  function ready(error, datapoints) {
    if (error || !datapoints || !datapoints.length) {
      console.error(error);
      d3.select(".Aligner").append("p").attr("class", "data-error").text(
    ...
    d3.select("#combine").on("click", function () {
      simulation.force("x", forceXCombine).force("y", forceYCombine).alphaTarget(0.1).restart();
    });
